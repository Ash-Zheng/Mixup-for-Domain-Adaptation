...
parser.add_argument('--aug_times', type=int, default=10)
...
device = 'cuda:1'
train_transform_mapping = {'mnist': transforms.Compose([  
    transforms.ToTensor(),
...
val_transform_mapping = {'mnist': transforms.Compose([  
    transforms.ToTensor(),
    transforms.Normalize(mean= (0.1307,), std= (0.3081,))]),
'mnistm': transforms.Compose([
    transforms.RandomCrop(28),
...
train_s_loader = train_dataloader_mapping[source]  
train_t_loader = train_dataloader_mapping[target]  
val_loader = val_dataloader_mapping[target]  
